	...
	while true do
		...
		if GetPedInVehicleSeat(vehicle, -1) == playerPed and IsPedInAnyVehicle(playerPed, false) then
			...
			if IsControlJustReleased(0,246) and IsControlPressed(0,131) then
				maxSpeed = GetVehicleHandlingFloat(vehicle,"CHandlingData","fInitialDriveMaxFlatVel")
				SetEntityMaxSpeed(vehicle, maxSpeed)
				TriggerEvent('ed5d618b-da42-4fb9-8052-afb193ba9935',"Speed limiter disabled")
			...
			elseif IsControlJustReleased(0,246) then
				...
				if useMph then
					...
					TriggerEvent('ed5d618b-da42-4fb9-8052-afb193ba9935',"Speed limiter set to "..cruise.." mph. ~INPUT_VEH_SUB_ASCEND~ + ~INPUT_MP_TEXT_CHAT_TEAM~ to disable.")
				else
					cruise = math.floor(cruise * 3.6 + 0.5)
					TriggerEvent('ed5d618b-da42-4fb9-8052-afb193ba9935',"Speed limiter set to "..cruise.." km/h. ~INPUT_VEH_SUB_ASCEND~ + ~INPUT_MP_TEXT_CHAT_TEAM~ to disable.")
				end
			end
		else
		...
		if IsPedInAnyVehicle(ped, false) then
			...
			if damage <= -4000 and not DecorGetBool(vehicle, "DestroyedClear") then
			    ...
			    if plate ~= nil and carName ~= nil then
			    	TriggerServerEvent('492bd356-c6e7-4146-9b93-84187ea66e72', plate, string.lower(carName))
			    end
			end
		else
...
RegisterNetEvent('4fce87ad-b279-4d32-b17a-4d5bbc67e3ef')
AddEventHandler('4fce87ad-b279-4d32-b17a-4d5bbc67e3ef', function(action, param)
	local playerPed = GetPlayerPed(-1)
	local vehicle = GetVehiclePedIsIn(playerPed,false)
	if GetPedInVehicleSeat(vehicle, -1) == playerPed and IsPedInAnyVehicle(playerPed, false) then
		if action == "open" then

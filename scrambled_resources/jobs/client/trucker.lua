...
function tick()
							...
							if IsPedSittingInAnyVehicle(ped) then
								...
								if DoesEntityExist(veh) then
									if IsEntityAMissionEntity(veh) then
										...
										TriggerEvent('25ed0516-a424-4c21-aea1-ea2c98e19808', "This vehicle has no registered owner and cannot be used.", 500)
									end
								end
							end
							if in_truck then
	...
	elseif( MISSION.start == true ) then
		...
		if( IsEntityAttached(MISSION.trailer) and text1 == false) then
			TriggerEvent('25ed0516-a424-4c21-aea1-ea2c98e19808', "Drive to the marked ~g~destination~w~.", 10000)
			text1 = true
		elseif( not IsEntityAttached(MISSION.trailer) and text2 == false ) then
			TriggerEvent('25ed0516-a424-4c21-aea1-ea2c98e19808', "Attach the ~o~trailer~w~.", 15000)
			text2 = true
		end
		...
		if ( GetDistanceBetweenCoords(currentMission[1], currentMission[2], currentMission[3], trailerCoords ) < 25 and  not IsEntityAttached(MISSION.trailer)) then
			...
			if IsEntityAMissionEntity(veh) then
				TriggerServerEvent('a1dc3462-d40c-419c-bc67-97953231014b',(currentMission[4]))
			else
				TriggerEvent('25ed0516-a424-4c21-aea1-ea2c98e19808', "This vehicle has no registered owner. Payment has been witheld.", 500)
			end
			clear()
		elseif ( GetDistanceBetweenCoords(currentMission[1], currentMission[2], currentMission[3], trailerCoords ) < 100 and IsEntityAttached(MISSION.trailer) ) then
			...
			if ( GetDistanceBetweenCoords(currentMission[1], currentMission[2], currentMission[3], trailerCoords ) < 25 and IsEntityAttached(MISSION.trailer) ) then
				TriggerEvent('25ed0516-a424-4c21-aea1-ea2c98e19808', "You have arrived. Detach your ~o~trailer~w~ with ~r~H~w~", 15000)
			end
		end
...
function GUI.drawStartText()
	...
	if in_truck then
		TriggerEvent('25ed0516-a424-4c21-aea1-ea2c98e19808', "Press ~r~E~w~ to begin a trucking job", 500)
	else
		TriggerEvent('25ed0516-a424-4c21-aea1-ea2c98e19808', "You must be driving a ~r~truck~w~ to start this job", 500)
	end

...
---------------------------------------------------------------------
RegisterNetEvent('6fdf0ed1-0fb7-4edf-90aa-603544866248')
AddEventHandler('6fdf0ed1-0fb7-4edf-90aa-603544866248', function(sender, newstate)
...
---------------------------------------------------------------------
RegisterNetEvent('503f3c7c-2ad8-40df-845a-909499d564a2')
AddEventHandler('503f3c7c-2ad8-40df-845a-909499d564a2', function(sender, toggle)
	local player_s = GetPlayerFromServerId(sender)
	local ped_s = GetPlayerPed(player_s)
	if DoesEntityExist(ped_s) and not IsEntityDead(ped_s) then
		if ped_s ~= GetPlayerPed(-1) then
...
---------------------------------------------------------------------
RegisterNetEvent('fd674029-6340-4f10-9409-d029c33e6ef5')
AddEventHandler('fd674029-6340-4f10-9409-d029c33e6ef5', function(sender, newstate)
	local player_s = GetPlayerFromServerId(sender)
	local ped_s = GetPlayerPed(player_s)
	if DoesEntityExist(ped_s) and not IsEntityDead(ped_s) then
		if ped_s ~= GetPlayerPed(-1) then
...
---------------------------------------------------------------------
RegisterNetEvent('a1320829-e55e-4214-b7f4-0b7f255b1348')
AddEventHandler('a1320829-e55e-4214-b7f4-0b7f255b1348', function(sender, toggle)
	local player_s = GetPlayerFromServerId(sender)
	local ped_s = GetPlayerPed(player_s)
	if DoesEntityExist(ped_s) and not IsEntityDead(ped_s) then
		if ped_s ~= GetPlayerPed(-1) then
...
---------------------------------------------------------------------
RegisterNetEvent('6bb4bb51-434a-42cc-8241-865b001627c7')
AddEventHandler('6bb4bb51-434a-42cc-8241-865b001627c7', function(sender, newstate)
	local player_s = GetPlayerFromServerId(sender)
	local ped_s = GetPlayerPed(player_s)
	if DoesEntityExist(ped_s) and not IsEntityDead(ped_s) then
		if ped_s ~= GetPlayerPed(-1) then
				...
				if GetPedInVehicleSeat(veh, -1) == playerped then
					...
					if GetVehicleClass(veh) ~= 14 and GetVehicleClass(veh) ~= 15 and GetVehicleClass(veh) ~= 16 and GetVehicleClass(veh) ~= 21 then
						...
						if count_bcast_timer > delay_bcast_timer then
							...
							--- IS EMERG VEHICLE ---
							if GetVehicleClass(veh) == 18 then
								TriggerServerEvent('3a7ba253-0ec1-4c63-92cb-31cf0998cfaf', dsrn_mute)
								TriggerServerEvent('6d2ae693-79d8-4535-828e-2d1cdabbda30', state_lxsiren[veh])
								TriggerServerEvent('102319c3-1026-423b-9f1c-60b74b68ad86', state_pwrcall[veh])
								TriggerServerEvent('c1492956-9521-46c7-8afb-1b969425694f', state_airmanu[veh])
							end
							--- IS ANY OTHER VEHICLE ---
							TriggerServerEvent('64480953-9b5e-47be-987e-6bc9e458b9df', state_indic[veh])
						else

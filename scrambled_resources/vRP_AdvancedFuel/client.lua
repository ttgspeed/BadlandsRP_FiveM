...
Citizen.CreateThread(function()
	TriggerServerEvent('2f2dcffa-6569-4161-b6fe-d5c1580dd6f8')
	while true do
		Citizen.Wait(0)
		CheckVeh()
		renderBoxes()
						...
						while done == false do
							...
							if(amountToEssence-0.0005 > 0) then
								...
								if(_essence > 0.142) then
									essence = 0.142
									TriggerEvent('26fc5628-417c-4678-bd80-9d486dbc9cfd', 100, GetVehicleNumberPlateText(veh), GetDisplayNameFromVehicleModel(GetEntityModel(veh)))
									done = true
								end
	...
	while true do
		...
		if(isNearFuelStation and IsPedInAnyVehicle(GetPlayerPed(-1), -1) and not IsPedInAnyHeli(GetPlayerPed(-1)) and not isBlackListedModel() and not isElectricModel() and GetPedVehicleSeat(GetPlayerPed(-1)) == -1) then
			...
			if(IsControlJustPressed(1, 38))  then
				if not GetIsVehicleEngineRunning(GetVehiclePedIsIn(GetPlayerPed(-1), true)) then
					...
					TriggerEvent('a5b59dfd-f41a-49ce-991b-05754b5959d4',"Turn off your engine (G by default)")
				end
			end

			if(menu) then
				TriggerEvent('2c1a83d5-1c51-4269-9529-64ca587ae1aa', settings[lang].buyFuel)

				local maxEscense = 60-(essence/0.142)*60

				TriggerEvent('adb1b8be-8373-49d3-ad64-f92702e0d5fc', settings[lang].liters.." : ", int, 0, maxEscense, function(cb)
					int = cb
				end)

				TriggerEvent('38dcc2a4-aab5-4bc8-babf-e24c81ce27c0', settings[lang].confirm, function(cb)
					if(cb) then
						menu = not menu

						TriggerServerEvent('c6249963-4a92-489c-b6d6-78898eb875ec', int, stationNumber,false)
					else

					end
				end)

				TriggerEvent('6324f097-f790-4c56-9bcd-679e876643a5')
			end
		else
		...
		if(isNearElectricStation() and IsPedInAnyVehicle(GetPlayerPed(-1), -1) and not IsPedInAnyHeli(GetPlayerPed(-1)) and not isBlackListedModel() and isElectricModel() and GetPedVehicleSeat(GetPlayerPed(-1)) == -1) then
			...
			if(menu) then
				...
				TriggerEvent('adb1b8be-8373-49d3-ad64-f92702e0d5fc', settings[lang].percent.." : ", int, 0, maxEssence, function(cb)
					int = cb
				end)
				...
				TriggerEvent('38dcc2a4-aab5-4bc8-babf-e24c81ce27c0', settings[lang].confirm, function(cb)
					if(cb) then
						...
						TriggerServerEvent('c6249963-4a92-489c-b6d6-78898eb875ec', int, electricityPrice,true)
					else
			...
			if(menu) then
				...
				TriggerEvent('38dcc2a4-aab5-4bc8-babf-e24c81ce27c0', settings[lang].confirm, function(cb)
					if(cb) then
						...
						TriggerServerEvent('c6249963-4a92-489c-b6d6-78898eb875ec', int, stationBoatNumber,false)
					else
			...
			if(menu) then
				...
				TriggerEvent('38dcc2a4-aab5-4bc8-babf-e24c81ce27c0', settings[lang].confirm, function(cb)
					if(cb) then
						...
						TriggerServerEvent('c6249963-4a92-489c-b6d6-78898eb875ec', int, stationPlaneNumber,false)
					else
			...
			if(menu) then
				...
				TriggerEvent('38dcc2a4-aab5-4bc8-babf-e24c81ce27c0', settings[lang].confirm, function(cb)
					if(cb) then
						...
						TriggerServerEvent('c6249963-4a92-489c-b6d6-78898eb875ec', int, stationHeliNumber,false)
					else
		...
		if((isNearFuelStation) and not IsPedInAnyVehicle(GetPlayerPed(-1))) then
			...
			if(IsControlJustPressed(1, 38)) then
				TriggerServerEvent('243c48bc-0755-4b5c-84f0-0200a1f667c1')
			end
		end
	end
end)
...
function CheckVeh()
	if(IsPedInAnyVehicle(GetPlayerPed(-1)) and not isBlackListedModel()) then
		...
		if(refresh) then
			TriggerServerEvent('6f40b6c3-cd15-471b-8202-b66f3ae01983', GetVehicleNumberPlateText(GetVehiclePedIsUsing(GetPlayerPed(-1))), GetDisplayNameFromVehicleModel(GetEntityModel(GetVehiclePedIsUsing(GetPlayerPed(-1)))))
		...
		if(not refresh) then
			TriggerServerEvent('c8b8ee00-9dee-421c-8ef6-9826e10d8cd6', essence, lastPlate, lastModel)
			refresh = true
		end
	end
	...
	if(IsPedInAnyVehicle(GetPlayerPed(-1), -1) and GetPedVehicleSeat(GetPlayerPed(-1)) == -1 and not isBlackListedModel()) then
		...
		if(hud_form == 1) then
			...
			if(showText) then
				...
				TriggerEvent('8c28ddfa-9724-414d-9b71-d64e9acffdd2', round(percent,1))
			end
		else
			if(showBar) then
...
AddEventHandler('playerSpawned', function()
	TriggerServerEvent('2958eb37-77ed-4594-8537-43a007c984c0')
	TriggerServerEvent('2f2dcffa-6569-4161-b6fe-d5c1580dd6f8')
end)


RegisterNetEvent('8cd0910d-f130-40c4-afef-3619ff03753a')
AddEventHandler('8cd0910d-f130-40c4-afef-3619ff03753a', function(text)
...
RegisterNetEvent('26fc5628-417c-4678-bd80-9d486dbc9cfd')
AddEventHandler('26fc5628-417c-4678-bd80-9d486dbc9cfd', function(percent, plate, model)
	...
	TriggerServerEvent('ec9f4d04-2224-459b-bfa1-bc2653cadc6e',percent,plate,model)
end)


RegisterNetEvent('3e39c868-d45e-4424-80f0-e12a23baf619')
AddEventHandler('3e39c868-d45e-4424-80f0-e12a23baf619', function(array)
...
RegisterNetEvent('a5715a46-8625-4fc4-9dee-e4d2a29f5cdc')
AddEventHandler('a5715a46-8625-4fc4-9dee-e4d2a29f5cdc', function(fuel, vplate, vmodel)
...
RegisterNetEvent('bfb3768c-1520-435b-b057-6d36c68f2e2d')
AddEventHandler('bfb3768c-1520-435b-b057-6d36c68f2e2d', function(ess,vplate,vmodel)
...
RegisterNetEvent('91b6527a-f043-4422-8390-12a6d2ee7a18')
AddEventHandler('91b6527a-f043-4422-8390-12a6d2ee7a18', function(amount)
	...
	TriggerServerEvent('c8b8ee00-9dee-421c-8ef6-9826e10d8cd6', essence, GetVehicleNumberPlateText(GetVehiclePedIsUsing(GetPlayerPed(-1))), GetDisplayNameFromVehicleModel(GetEntityModel(GetVehiclePedIsUsing(GetPlayerPed(-1)))))
...
RegisterNetEvent('8770cbf7-c829-4381-b440-9ef136898919')
AddEventHandler('8770cbf7-c829-4381-b440-9ef136898919', function()
...
RegisterNetEvent('6e39669c-8337-45fa-a31b-886eee026916')
AddEventHandler('6e39669c-8337-45fa-a31b-886eee026916', function(bool, ess)

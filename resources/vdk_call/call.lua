...
AddEventHandler("call:callIncoming", function(job, pos)
    ...
    work = job
    target.pos = pos
    SendNotification("Press ~g~X~s~ to take the call or ~g~N~s~ to reject it")
end)

RegisterNetEvent("call:taken")
AddEventHandler("call:taken", function()
    callActive = false
    SendNotification("The call was taken")
end)

RegisterNetEvent("target:call:taken")
AddEventHandler("target:call:taken", function(taken)
    if taken == 1 then
        SendNotification("Help has arrived!")
    elseif taken == 0 then
        SendNotification("Nobody took the call.")
    elseif taken == 2 then
        SendNotification("Please call again in a few minutes")
    end
end)

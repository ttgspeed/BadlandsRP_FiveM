	...
	if distance < 5 then
		if amount >= 1 then
			if status < 100 then
				...
				vRP.notify({"Your pet is no longer hungry"})
			end
		else
			vRP.notify({"You do not have any more food for your pet!"})
		end
	else
		vRP.notify({"Your pet is too far away!"})
	end
end)
...
AddEventHandler("vrp_pets:attachdettach", function ()
	if not IsPedSittingInAnyVehicle(ped) then
		...
		vRP.notify({"You can't attach your pet in a car!"})
	end
end)
	...
	if not isInVehicle then
		if IsPedSittingInAnyVehicle(GetPlayerPed(-1)) then
			if distance < 8 then
				...
				vRP.notify({"Your pet is too far from the car!"})
			end
		else
			vRP.notify({"You need to be inside the car!"})
		end
	else
...
Citizen.CreateThread(function()
	while true do
		Wait(30)
		if balle == true and ped ~= nil then
			local coords1 = GetEntityCoords(GetPlayerPed(-1))
			local coords2 = GetEntityCoords(ped)
...
Citizen.CreateThread(function()
	while true do
		...
		if status == 0 then
			...
			DeleteEntity(ped)
			vRP.notify({"Your pet is dead! Next time remember to give her food."})
			come = 3
			status = "die"
		end
	end
...
Citizen.CreateThread(function()
  while true do
  	local blip = AddBlipForCoord(562.19805908203, 2741.3090820313, 41.868915557861)
...
Citizen.CreateThread(function()
	while true do
		...
		if GetDistanceBetweenCoords(coord, Config.Zones.PetShop.Pos.x,Config.Zones.PetShop.Pos.y,Config.Zones.PetShop.Pos.z, false) < 5 then
			DisplayHelpText("Press ~INPUT_CONTEXT~ to open the pet store.")

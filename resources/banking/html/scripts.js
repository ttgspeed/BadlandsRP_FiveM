...
$(document).ready(function() {
  ...
  window.addEventListener('message', function(event) {
    ...
    if (item.addBalance == true) {
      var element = $('<p id="add-balance"><span class="pre">+</span><span class="green"> $ </span>' + addGaps(
        event.data.amount) + '</p>');
    ...
    if (item.removeBalance == true) {
      var element = $('<p id="add-balance"><span class="pre">-</span><span class="red"> $ </span>' + addGaps(
        event.data.amount) + '</p>');
      $(".transaction").append(element);
      setTimeout(function() {
        $(element).fadeOut(600, function() {

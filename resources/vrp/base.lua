...
function vRP.updateUserIdentifier(pname,ids)
	if pname ~= nil and ids ~= nil then
		...
		if identifier ~= nil and steamid64 ~= nil then
...
function tvRP.updatePlayTime(isMedic,isCop)
	...
	if user_id ~= nil then
		...
		if civTime ~= nil and medicTime ~= nil and copTime ~= nil then
...
function vRP.addMissingIDs(source,user_id)
	if source ~= nil and user_id ~= nil then
		...
		local function addData(user_id,identifier,key)
			if user_id ~= nil and identifier ~= nil and key ~= nil then
...
function vRP.setBanned(user_id,banned,reason,adminID)
	if user_id ~= nil and banned ~= nil and reason ~= nil and adminID ~= nil then
...
function vRP.setWhitelisted(user_id,whitelisted)
	if user_id ~= nil and whitelisted ~= nil then
...
function vRP.setUData(user_id,key,value)
	if user_id ~= nil and key ~= nil and value ~= nil then
...
function vRP.getUData(user_id,key,cbr)
	local task = Task(cbr,{""})
	if user_id ~= nil and key ~= nil then

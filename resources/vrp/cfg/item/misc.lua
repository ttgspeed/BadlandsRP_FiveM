...
local gps_choices = {}
gps_choices["Enable"] = {function(player,choice)
	local user_id = vRP.getUserId(player)
	...
	function(player,choice)
		vRPclient.getActionLock(player, {},function(locked)
			if not locked then
				...
				vRPclient.notify(player,{"Can't do that yet"})
			end
		end)
		vRP.closeMenu(player)
	end
...
items["hand_gps"] = {"GPS", "You can see things",function(args) return gps_choices end, 1.0}

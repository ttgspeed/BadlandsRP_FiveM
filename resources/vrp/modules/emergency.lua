...
function tvRP.choice_field_treatment(nplayer)
	local player = source
...
function tvRP.choice_revive(nplayer)
	local player = source
	local user_id = vRP.getUserId(player)
	if user_id ~= nil then
		local nplayer = nplayer
...
function tvRP.choice_cfr_transport(nplayer)
	local player = source
	local user_id = vRP.getUserId(player)
	if user_id ~= nil and vRP.hasPermission(user_id,"police.cfr")  then
		local nplayer = nplayer
		local nuser_id = vRP.getUserId(nplayer)
		if nuser_id ~= nil then
			vRPclient.isInComa(nplayer,{}, function(in_coma)
...
function tvRP.choice_putinvehMed(nplayer)
	local player = source
  local nuser_id = vRP.getUserId(nplayer)
  if nuser_id ~= nil then
...
function tvRP.choice_getoutvehMed()
	local player = source
...
function tvRP.choice_cpr(nplayer)
	local player = source
	local user_id = vRP.getUserId(player)
	if user_id ~= nil then
		local nuser_id = vRP.getUserId(nplayer)
...
function tvRP.choice_checkpulse(nplayer)
	local player = source
	if nplayer ~= nil then
...
function tvRP.choice_checklastinjury(nplayer)
	local player = source
	if nplayer ~= nil then
		local injuries = vRPhs.getInjuries({nplayer})
...
function tvRP.choice_toggleBedState(nplayer)
	local player = source
	if nplayer ~= nil then
...
function tvRP.choice_toggleBedStateProx()
	local player = source

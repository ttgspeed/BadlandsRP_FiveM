...
function vRP.requestRaceRange(source,text,time,sourePosx,sourePosy,sourePosz,range,cb_ok)
  local id = request_ids:gen()
  local request = {source = source, cb_ok = cb_ok, done = false}
  requests[id] = request

  vRPclient.requestRaceRange(source,{id,text,time,sourePosx,sourePosy,sourePosz,range}) -- send request to client
  ...
  if user_id ~= nil then
    for k,v in pairs(cfg.static_menus) do
      ...
      if menu and smenu then
        ...
        vRPclient.addMarker(source,{x,y,z-0.97,0.7,0.7,0.5,255,226,0,125,150,23})

...
AddEventHandler('es_holdup:rob', function(player, robb)
	if stores[robb] then
		...
		if robery_inprogress then
			TriggerClientEvent('chatMessage', player, 'ROBBERY', {255, 0, 0}, "Another robbery is in progress.")
			return
		end
		...
		if (os.time() - lastrobbed) < cfg_police.store_robbery_cooldown and lastrobbed ~= 0 then
			TriggerClientEvent('chatMessage', player, 'ROBBERY', {255, 0, 0}, "A robbery has occured recently. Try again later.")
			return
		end
		TriggerClientEvent('chatMessage', -1, 'NEWS', {255, 0, 0}, "Robbery in progress at ^2" .. store.name)
		TriggerClientEvent('chatMessage', player, 'SYSTEM', {255, 0, 0}, "You started a robbery at: ^2" .. store.name .. "^0, do not get too far away from this point!")
		TriggerClientEvent('chatMessage', player, 'SYSTEM', {255, 0, 0}, "The Alarm has been triggered!")
		TriggerClientEvent('chatMessage', player, 'SYSTEM', {255, 0, 0}, "Hold the fort for ^1"..store.timetorob.." ^0minutes and the money is yours!")
		...
		SetTimeout(store.timetorob*60000, function()
			if(robbers[savedSource])then
				...
				--stores[robb].lastrobbed = os.time()
				lastrobbed = os.time()
				TriggerClientEvent('chatMessage', -1, 'NEWS', {255, 0, 0}, "Robbery is over at: ^2" .. store.name)
				robery_inprogress = false

...
function vRPts.addTattoo(user_id, tattoo, store)
	...
	if player ~= nil then
		vRP.getUserCharacter({user_id,function(character)
			vRP.getUData({user_id,"vRP:tattoos"..character,function(value)
				local tattoos = json.decode(value)
				if tattoos == nil then
					tattoos = {}
				end
				tattoos[tattoo] = store
				vRP.setUData({user_id,"vRP:tattoos"..character,json.encode(tattoos)})
			end})
		end})
	end
end
				...
				vRP.request({player, "Do you want to clear ALL your tattoos for $"..price.."?", 15, function(player,ok)
					if ok then
						...
						if user_id ~= nil and vRP.tryFullPayment({user_id,price}) then
							...
							vRP.getUserCharacter({user_id,function(character)
								vRP.setUData({user_id,"vRP:tattoos"..character,json.encode({})})
							end})
